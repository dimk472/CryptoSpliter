...
import MyEvents from "../components/events/MyEventsTab.tsx";
import { prepareContractCall, sendTransaction, waitForReceipt } from "thirdweb";
import { getEventContract } from "../blockchain/contract.ts";
import '../components/styles/CryptoSpliter.css';
import '../components/styles/splittingApp.css';
import { parseEther } from "ethers";
import LoadingEffect from '../components/loading/LoadingEffect.tsx';
import { useContacts } from '../components/contacts/useContacts';
import ContactPicker from '../components/contacts/ContactPicker';
import ContactsManager from '../components/contacts/ContactsManager';
import ContactsSidebar from '../components/contacts/ContactSidebar';
import { Save } from 'lucide-react';
import '../components/styles/Contacts.css';
import '../components/styles/contacts-sidebar.css';
...
function SplittingApp({ walletAddress }: { walletAddress: string }) {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (event: KeyboardEvent) => {
            if (event.key === 'Escape') setIsNetworkOpen(false);
        };
    ...
    const handleSelectChain = async (targetChain: (typeof SUPPORTED_CHAINS)[number]) => {
        setIsNetworkOpen(false);
        if (targetChain.id === activeChain?.id) return;

        try {
            setIsSwitchingChain(true);
            await switchChain(targetChain);
        } catch (err) {
            alert(err instanceof Error ? err.message : 'Failed to switch network in wallet.');
        } finally {
    ...
    const goStep2 = () => {
        ...
        if (!activeChain) { alert('Please select a network.'); return; }
    ...
    const handleSelectContact = (contact: any, address: string, chainId: number) => {
        const emptyParticipant = participants.find(p => !p.isYou && !p.name.trim());
        if (emptyParticipant) {
            updateParticipant(emptyParticipant.id, 'name', contact.name);
            updateParticipant(emptyParticipant.id, 'address', address);
        }
    };
    ...
    return (
        <div className={`sa-wrap reveal-scale${isVisible ? ' is-visible' : ''}`} ref={ref}>
                            ...
                            {currentStep === 1 && (
                                <div>
                                    ...
                                    <div className="field-group">
                                        ...
                                        <div className="network-select" ref={networkRef}>
                                            <button
                                                type="button"
                                            ...
                                            >
                                                {activeChain ? (
                                                    <ChainProvider chain={activeChain}>
                                                        <ChainIcon
                                                            ...
                                                            loadingComponent={<span className="network-icon-placeholder" />}
                                                            fallbackComponent={<span className="network-icon-placeholder" />}
                                                        />
                                                    </ChainProvider>
                                                ) : (
                                                ...
                                                <span className={`network-caret${isNetworkOpen ? ' network-caret--open' : ''}`} />
                                            </button>

                                            {isNetworkOpen && (
                                                <ul className="network-select-list" role="listbox">
                                                    {SUPPORTED_CHAINS.map((chain) => (
                                                        <li key={chain.id}>
                                                            <button
                                                                type="button"
                                                                className={`network-option${activeChain?.id === chain.id ? ' network-option--active' : ''}`}
                                                            ...
                                                            >
                                                                ...
                                                                <span className="network-option-label">{chain.name}</span>
                                                                {activeChain?.id === chain.id && (
                                                                    <span className="network-option-check">✓</span>
                                                                )}
                                                            </button>
                                                        </li>
                                                    ))}
                                        ...
                                        {isSwitchingChain && (
                                            <p className="field-hint">Confirm the network switch in your wallet…</p>
                                        )}
                                    </div>
                                                ...
                                                {!participant.isYou && (
                                                    <ContactPicker
                                                        ...
                                                        onSelect={(contact, address) => {
                                                            updateParticipant(participant.id, 'name', contact.name);
                                                            updateParticipant(participant.id, 'address', address);
                                                        }}
                                                    />
                                                )}
            ...
            {managerOpen && (
                <ContactsManager
                    ...
                    initialEditingContact={editingContact}
                />
            )}
        </div>
    );
            ...
            <div className="hiw-container">
                ...
                <div className="hiw-cards-wrapper">
                    <div className="how-it-works-cards">
                        <img src={card1} alt="Step 1: Connect your crypto wallet to CryptoSplitter" />
                    </div>
                    <div className="how-it-works-cards">
                        <img src={card2} alt="Step 2: Create a split event and add participants with shares" />
                    </div>
                    <div className="how-it-works-cards">
                        <img src={card3} alt="Step 3: Settle your share with an on-chain payment from your wallet" />
                    </div>
                </div>
            </div>
        </section>
                    ...
                    <div className="hero-wrap">
                        <header className="header" id="header">
                            <div className="header-container">
                                <a href="#home" className="logo">
                                    <img src={Icon} alt="CryptoSpliter Logo" />
                                </a>
                                ...
                                <nav className="nav" id="nav" ref={navRef}>
                                    ...
                                    <ul
                                        ...
                                        id="nav-list"
                                    >
                                        <li><a href="#hero" className="nav-link" onClick={closeMobileNav}>Home</a></li>
                                        <li><a href="#my-events" className="nav-link" onClick={closeMobileNav}>My Events</a></li>
                                        <li><a href="#learn" className="nav-link" onClick={closeMobileNav}>Learn</a></li>
                        ...
                        <div className="hero-inner">
                            ...
                            <div className="hero-left">
                                ...
                                <p className="hero-meta">0.5% platform fees · Works with the wallet you already use</p>
                            </div>
                        ...
                        <div className="footer-main">
                            <div className="footer-col footer-col--brand">
                                ...
                                <div className="footer-social">
                                    <a href="https://www.linkedin.com/company/cryptosplitter/" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">
                            ...
                            <div className="footer-col">
                                <h4 className="footer-title">Tokens</h4>
                                <ul className="footer-list">
                                    <li><Link to="/split-eth">Split ETH</Link></li>
                                    <li><Link to="/split-usdc">Split USDC</Link></li>
                            ...
                            <div className="footer-col">
                                <h4 className="footer-title">Chains</h4>
                                <ul className="footer-list">
                                    <li><Link to="/split-base">Split on Base</Link></li>
                            ...
                            <div className="footer-col">
                                ...
                                <ul className="footer-list">
                                    <li><Link to="/split-splitwise-crypto-alternative">Splitwise Alternative</Link></li>
                                    <li><Link to="/split-vacation">Vacation Expenses</Link></li>
                                    <li><Link to="/split-roommates">Roommate Bills</Link></li>
                                </ul>
                            </div>
                            ...
                            <div className="footer-col">
                                ...
                                <ul className="footer-list">
                                    ...
                                    <li><a href="/GDPRCompliance/gdpr.html">GDPR Compliance</a></li>
